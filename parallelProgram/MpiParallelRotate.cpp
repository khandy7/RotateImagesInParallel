...
int main(int argc, char* argv[]) {
    ...
    //need barrier for proper timing
    MPI_Barrier(MPI_COMM_WORLD);
    fstart = MPI_Wtime();

    //divide cards up into intervals based on number of nodes provided
    //number of nodes should be divisible by 52 for best results
    int cardsPerNode = 52/comm_sz;

    //total number of cards for each rank
    int numCards = my_rank*cardsPerNode;

    //----------------->START CARD ROTATIONS<------------------------
    //for loop loops through current ranks cards, rotates them one at a time
    for (int e = numCards; e < numCards+cardsPerNode; e++) {
